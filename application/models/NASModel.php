	...
	class NASModel extends CI_Model
	{
		...
		public function getNas($id){
			if($id=='All'){
				...
				$query=$this->db->query("SELECT * from nas inner join department on nas.DepartmentID=department.DepartmentID where department.DepartmentID='$id' and nas.IsDeleted=0");
				if($query){
					return $query->result_array();
				}else{
					return false;
